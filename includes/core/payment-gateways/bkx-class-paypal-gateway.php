<?php
/**
 * BookingX Core Payment Gateway Class Load.
 *
...
class BkxPaymentPayPalExpress {
	...
	/**
	 * BkxPaymentPayPalExpress constructor.
	 *
	 * @param null $order_id
	 */
	...
	public function paypal_request_args( $method = 'SetExpressCheckout', $args = null ) {
		...
		$service_name = $bkx_baseObj->get_title();
		...
		} elseif ( $method == $this->doMethod && isset( $_GET['token'], $_GET['PayerID'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
			$get_token    = sanitize_text_field( wp_unslash( $_GET['token'] ) ); // phpcs:ignore WordPress.Security.NonceVerification
			$get_payer_id = sanitize_text_field( wp_unslash( $_GET['PayerID'] ) ); // phpcs:ignore WordPress.Security.NonceVerification
	...
	/**
	 * @param $order_id
	 * @param $generate_paypal_response_data
	 *
	 * @return array
	 */
	public function ready_for_redirect_to_paypal( $order_id, $generate_paypal_response_data ) {
		...
		if ( 'SUCCESS' == strtoupper( $token_ack_response['ACK'] ) || 'SUCCESSWITHWARNING' == strtoupper( $token_ack_response['ACK'] ) ) {
			...
			echo '<meta http-equiv="refresh" content="0;url=' . esc_url( $paypalurl ) . '">';
		} else {
			// Show error message.
		...
		if ( isset( $get_token ) && isset( $get_payer_id ) ) {
			...
			if ( ! empty( $response ) && 'SUCCESS' == strtoupper( $response['ACK'] ) || 'SUCCESSWITHWARNING' == strtoupper( $response['ACK'] ) ) {
				...
				if ( 'SUCCESS' == strtoupper( $getTransaction['ACK'] ) || 'SUCCESSWITHWARNING' == strtoupper( $getTransaction['ACK'] ) ) {
					...
					return $process_response = array(
						'success' => true,
						'data'    => $final_payment_meta,
					);
				} else {
					return $getTransaction;
				}
