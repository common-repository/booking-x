<?php
/**
 * BookingX Core Payment Gateway Core Class Load.
 *
...
class BkxPaymentCore {
	...
	public function bkx_paypal_payment_gateway_action( $args ) {
		...
		if ( $order_id != '' && isset( $_POST['bkx_payment_gateway_method'] ) && sanitize_text_field( wp_unslash( $_POST['bkx_payment_gateway_method'] ) ) == 'bkx_gateway_paypal_express'
			...
			if ( ! empty( $payment_request_call ) && ! isset( $payment_request_call['error'] ) ) {
				...
				if ( $method == 'SetExpressCheckout' && 'SUCCESS' == strtoupper( $payment_request_call['ACK'] ) || 'SUCCESSWITHWARNING' == strtoupper( $payment_request_call['ACK'] ) ) {
					echo '<h3>' . esc_html__( 'Please wait Booking Processing, you will be forwarded to PayPal soon....', 'bookingx' ) . '</h3>';
					...
					echo '<h3>' . esc_html__( 'Please wait redirecting soon..', 'bookingx' ) . '</h3>';
					echo '<meta http-equiv="refresh" content="0;url=' . esc_url( $bkx_return_url ) . '">';
					die();
				}
			}
		} elseif ( $order_id != '' && ! isset( $_POST['bkx_payment_gateway_method'] ) ) {
			update_post_meta( $order_id, 'bkx_capture_payment', $process_response );
			$bkx_return_url = get_permalink( bkx_crud_option_multisite( 'bkx_plugin_page_id' ) );
			$bkx_return_url = apply_filters( 'bkx_return_url', $bkx_return_url );
			$bkx_return_url = add_query_arg( array( 'order_id' => base64_encode( $order_id ) ), $bkx_return_url );
	...
	public function bkx_paypal_payment_gateway_capture_action() {
		...
		if ( isset( $_GET['token'] ) && isset( $_GET['PayerID'] ) ) { // phpcs:disable WordPress.Security.NonceVerification
			$token                   = sanitize_text_field( wp_unslash( $_GET['token'] ) );  // phpcs:disable WordPress.Security.NonceVerification
			$payerid                 = sanitize_text_field( wp_unslash( $_GET['PayerID'] ) ); // phpcs:disable WordPress.Security.NonceVerification
	...
	public function default_payment_gateway() {
		...
		if ( isset( $check_status ) && $check_status == 1 ) {
			$load_gateways = array(
				'bkx_gateway_paypal_express' => array(
					'title' => 'Paypal',
					'class' => new BkxPaymentPayPalExpress(),
				),
			);
		}
	...
	public function bkx_get_available_gateways( $post = array() ) {
		...
		if ( isset( $post['seat_id'] ) && $post['seat_id'] > 0 ) {
			$seat_id          = sanitize_text_field( wp_unslash( $post['seat_id'] ) ); // phpcs:ignore WordPress.Security.NonceVerification
			$seatIsPrePayment = get_post_meta( $seat_id, 'seatIsPrePayment', true );
			if ( isset( $seatIsPrePayment ) && sanitize_text_field( $seatIsPrePayment ) == 'Y' ) {
				$prepayment = true;
			} else {
				$prepayment = false;
			}
	...
	public function bkx_get_available_gateways_html( $prepayment = false ) {
		if ( ! empty( $this->payment_gateways ) ) {
			...
			$count                    = ! empty( $this->payment_gateways ) ? count( $this->payment_gateways ) : 0;
	...
	public function bkx_success_payment( $booking_data ) {
		...
		if ( $booking_data['success'] == true || $booking_data['success'] == 1 ) {
			...
			if ( strtolower( $payment_status ) == 'completed' ) {
				...
				$payment_success_html = sprintf( __( '<h2>Dear %1$s, Transaction Complete, but payment is still pending! You need to manually authorize this payment in your <a target="_new" href="%2$s">PayPal Account</a></h2>', 'bookingx' ), $booking_meta_data['first_name'], esc_url( 'http://www.paypal.com' ) );
			}
			...
			if ( isset( $order_id ) ) {
				...
				$base_name        = str_replace( ')', '', $base_name );
				$date_format      = '';
				$end_time         = date( 'H:i', strtotime( $booking_meta_data['booking_end_date'] ) ); // phpcs:ignore WordPress.DateTime.RestrictedFunctions.date_date
				...
				if ( isset( $base_time_option ) && $base_time_option == 'D' ) {
					...
					$date_format      = date( 'F d, Y', strtotime( $booking_meta_data['booking_date'] ) ); // phpcs:ignore WordPress.DateTime.RestrictedFunctions.date_date
				...
				$payment_success_html .= '</div></div>';
				if ( $bkx_seatObj->seat_address() ) {
					$payment_success_html .= '<div class="row"><div class="col-sm-12 mt-3">';
					$payment_success_html .= __( "{$bkx_seatObj->seat_address_html(false)}", 'bookingx' );
					$payment_success_html .= '</div></div>';
				}
