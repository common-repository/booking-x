...
if ( ! function_exists( 'bookingx_breadcrumb' ) ) {
	...
	function bookingx_breadcrumb( $args = array() ) {
		...
		if ( ! empty( $args['home'] ) ) {
			$breadcrumbs[] = array(
				'title'     => $args['home'],
				'hyperlink' => apply_filters( 'bookingx_breadcrumb_home_url', home_url() ),
			);
		}
		if ( is_search() ) {
			$breadcrumbs[] = array(
				'title'     => booking_x_page_title(),
				'hyperlink' => '',
			);
		} elseif ( is_tax() ) {
			$breadcrumbs[] = array(
			...
			$breadcrumbs[] = array(
				...
				'hyperlink' => get_post_type_archive_link( $post_type ),
			);
		} elseif ( is_singular() ) {
			$post_type     = get_post_type();
...
function bkx_get_formatted_price( $price ) {
	...
	if ( ! empty( $price ) ) :
		$currencyBlock = '<span class="currencyBlock"><currency itemprop="priceCurrency price" content="' . $price . '" > ' . get_bookingx_currency_symbol( $currency_option ) . '' . $price . '</currency></span>';
	endif;
...
if ( ! function_exists( 'booking_x_page_title' ) ) {
	...
	function booking_x_page_title( $echo = true ) {
		...
		$page_title = apply_filters( 'booking_x_page_title', esc_html( $page_title ) );

		if ( $echo ) {
			echo $page_title; // phpcs:disable WordPress.XSS.EscapeOutput.OutputNotEscaped
		} else {
			return $page_title;
		}
	}
...
if ( ! function_exists( 'bookingx_base_meta_data_html' ) ) {
	...
	function bookingx_base_meta_data_html( $bkx_base ) {
		...
		echo $BaseObj->get_meta_details_html( $base_id ); // phpcs:disable WordPress.XSS.EscapeOutput.OutputNotEscaped
	}
}
