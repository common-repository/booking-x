...
class BkxBooking {
	...
	public function __construct( $booking_set_array = null, $order_id = null ) {
		if ( ! empty( $booking_set_array ) ) {
			...
			$bkx_seat      = new BkxSeat( $seat_post );
			$bkx_base      = new BkxBase( $base_post );
			$bkx_extra_obj = array();
			if ( ! empty( $extra_ids ) ) {
				foreach ( $extra_ids as $key => $value ) {
					$extra_post      = get_post( $value );
					$bkx_extra_obj[] = new BkxExtra( $extra_post );
				}
			}
			// phpcs:disable WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
			$this->BkxSeat  = $bkx_seat;
			$this->BkxBase  = $bkx_base;
			$this->BkxExtra = $bkx_extra_obj;
	...
	public function get_calculate_total() {
		 ...
		 * Get Base price
		 */
		$bkx_base   = $this->BkxBase;
		$base_price = $bkx_base->get_price();
	...
	public function bkx_on_all_status_transitions( $new_status, $old_status, $post ) {
		$post_id = $post->ID;
		if ( $new_status !== $old_status && 'bkx_booking' === get_post_type( $post ) ) {
			if ( 'trash' === $new_status ) {
	...
	public function booking_cancelled() {
		...
		if ( isset( $order_status ) && '' === $order_status || 'Completed' === $order_status ) {
			return;
		}
	...
	public function booking_email( $booking_id, $status ) {
		...
		if ( $status ) {
			...
			$email_status = esc_html( "bkx_email_{$status}_booking" ); // bkx_email_bkx-pending_booking.
	...
	function CalendarJsonData( $search ) {
		...
		if ( ! empty( $get_posts ) ) {
			...
			foreach ( $get_posts as $key => $order_meta_data ) {
				...
				$generate_json       .= wp_json_encode( $generate_info ) . ',';
			}
	...
	public function GetBookedRecords( $search = null ) {
		...
		if ( isset( $search['service_id'] ) && $search['service_id'] != '' ) {
			...
			$seat_by_base = $bkx_base->get_seat_by_base();
		}
		...
		if ( $bookedresult->have_posts() ) :
			while ( $bookedresult->have_posts() ) :
				$bookedresult->the_post();
				$order_id = get_the_ID();
				if ( ! empty( $order_id ) ) {
	...
	public function get_order_meta_data( $order_id = null ) {
		...
		if ( isset( $seat_id ) && $seat_id != '' ) {
			$bkx_seat                    = new BkxSeat( '', $seat_id );
			$seat_arr                    = array(
				'main_obj'  => $bkx_seat,
				'title'     => $bkx_seat->get_title(),
				'permalink' => get_edit_post_link( $bkx_seat->id ),
			);
		...
		if ( isset( $base_id ) && $base_id != '' ) {
			...
			$base_arr                    = array(
				'main_obj'  => $bkx_base,
				'title'     => $bkx_base->get_title(),
				'permalink' => get_edit_post_link( $bkx_base->id ),
			);
		...
		if ( isset( $addition_ids ) && $addition_ids != '' ) {
			...
			if ( ! empty( $extra_obj ) ) {
				...
				foreach ( $extra_obj as $key => $extra ) {
					$extra_arr[] = array(
						'main_obj'  => $extra,
						'title'     => $extra->get_title(),
						'permalink' => get_edit_post_link( $extra->id ),
					);
				}
			}
	...
	function MakeBookingProcess( $data, $ajax = true ) {
		...
		if ( isset( $_POST['seat_id'], $_POST['base_id'], $_POST['starting_slot'], $_POST['time_option'] ) ) {
			...
			if ( ! empty( $get_verify_slot ) && 1 === $get_verify_slot->result || ! empty( $args['booking_multi_days'] ) ) {
				...
				if ( isset( $_POST['edit_booking'] ) && true === $_POST['edit_booking'] ) {
			...
			if ( empty( $args['time'] ) && 'H' === $args['time_option'] ) {
				$result = __( 'BTB', 'bookingx' );
			} elseif ( empty( $args['booking_multi_days'] ) && 'D' === $args['time_option'] ) {
				$result = __( 'BTB', 'bookingx' );
			} else {
				$result = ( ! empty( $booking['meta_data']['order_id'] ) ? wp_json_encode( $booking ) : 'NORF' );
			}
		} else {
			$result = __( 'SWR', 'bookingx' );
		}
		if ( true === $ajax ) {
			echo $result; // phpcs:disable WordPress.XSS.EscapeOutput.OutputNotEscaped
		} else {
			return $result;
		}
	}
	...
	public function get_verify_slot( $args, $echo = true ) {
		...
		$range_total       = count( $availability_slots['range'] );
			...
			if ( ! empty( $objBookigntime ) ) :
					...
					if ( $seatslots > 1 ) {
						for ( $i = 0; $i < $seatslots; $i ++ ) {
							...
							$checked_booked_slots[] = array(
								'slot_id'  => $objBooknig['full_day'] + $i,
								'order_id' => $objBooknig['booking_record_id'],
							);
						}
					} else {
				...
				if ( ! empty( $get_booked_range ) ) :
							...
							foreach ( $get_booked_slot as $seat_id => $booked_slot ) {
								foreach ( $find_slot_value_in_booked_array as $_find_slot ) {
									if ( in_array( $_find_slot, $booked_slot ) ) {
										$total_booked_slot_count                      = count( $booked_slot );
					...
					if ( ! empty( $seat_by_base ) ) {
						$total_free_seat_all = count( $seat_by_base );
					...
					if ( ! empty( $total_frees_seats ) && count( $total_frees_seats ) > 1 ) {
					...
					if ( ! empty( $free_seat_all_arr ) && count( $free_seat_all_arr ) > 1 ) {
		...
		if ( $echo == true ) {
			...
			return $output; // phpcs:disable WordPress.XSS.EscapeOutput.OutputNotEscaped
		}
		wp_die();
	...
	public function get_display_availability_slots( $args ) {
		...
		$current_order_id     = isset( $_POST['order_id'] ) && $_POST['order_id'] != '' ? sanitize_text_field( wp_unslash( $_POST['order_id'] ) ) : 0;
		$checked_booked_slots = array();
		...
		if ( ! empty( $BookingTime ) ) {
			foreach ( $BookingTime as $temp ) {
				...
				if ( bkx_crud_option_multisite( 'enable_any_seat' ) == 1
					 && bkx_crud_option_multisite( 'select_default_seat' ) != ''
					 && isset( $_POST['seat_id'] ) && sanitize_text_field( wp_unslash( $_POST['seat_id'] ) ) == 'any'
				) :
					...
					if ( $seat_slots > 1 ) {
						for ( $i = 0; $i < $seat_slots; $i ++ ) {
							if ( isset( $temp['full_day'] ) ) {
								if ( isset( $temp['user_time_zone'], $temp['user_selected_slot'], $temp['user_booking_time'] ) && $temp['user_time_zone'] != '' && $temp['user_selected_slot'] != '' && $temp['user_time_zone'] == $args['user_time_zone'] ) {
									$booking_slot_arr[] = (int) $temp['user_selected_slot'] + $i;
								} else {
									$booking_slot_arr[] = (int) $temp['full_day'] + $i;
								}
								$checked_booked_slots[] = array(
									'created_by' => $PostObj->post_author,
									'slot_id'    => $temp['full_day'] + $i,
	...
	public function get_booking_form_calendar_availability( $args ) {
		...
		if ( isset( $args['base_id'] ) && $args['base_id'] != '' ) {
			...
			$bkx_base                = new BkxBase( '', $base_id );
			$get_service_unavailable = $bkx_base->get_service_unavailable();
		}
	...
	public function generate_order( $form_post_data, $order_id = null, $send_mail = true ) {
		...
		if ( ! isset( $order_id ) || $order_id == '' || $order_id == 0 ) {
			...
			$order_data['post_title']    = sprintf( __( 'Order &ndash; %s', 'bookingx' ), strftime( _x( '%1$b %2$d, %Y @ %I:%M %p', 'Order date parsed by strftime', 'bookingx' ) ) );
			...
			$wpdb->query(
				$wpdb->prepare(
					...
					'closed',
					uniqid( 'order_' ),
					sprintf( __( 'Order &ndash; %s', 'bookingx' ), strftime( _x( '%1$b %2$d, %Y @ %I:%M %p', 'Order date parsed by strftime', 'bookingx' ) ) ),
	...
	function UpdateBookingData( $order_id, $post_data ) {
		...
		//phpcs:enable WordPress.DateTime.RestrictedFunctions
		...
		wp_update_post(
			array(
				'ID'          => $order_id,
				'post_status' => 'bkx-' . apply_filters(
					'bkx_default_order_status',
					'pending'
				),
			)
		);
	...
	function CreateBookingData( $post_data ) {
		...
		$booking_set_array = array(
			'seat_id'  => sanitize_text_field( $post_data['seat_id'] ),
			'base_id'  => sanitize_text_field( $post_data['base_id'] ),
			'extra_id' => ( ! empty( $post_data['extra_id'] ) && $post_data['extra_id'] != 'None' ) ? implode( ',', $post_data['extra_id'] ) : '',
		);
		$total_tax         = 0;
		$total_price       = 0;
	...
	public function generate_order_meta( $post_data, $send_mail = true ) {
		...
		$GLOBALS['bkx_booking_data'] = array(
			'meta_data' => $bkx_booking_data,
			'time_data' => $bookingTimeRecord,
		);
	...
	public function bkx_bookingTimeRecord( $bookingTimeRecord ) {
		...
		if ( $minutes_temp == '00' ) {
			$counter = 0;
		} elseif ( $minutes_temp == '15' ) {
			$counter = 1;
		} elseif ( $minutes_temp == '30' ) {
			$counter = 2;
		} elseif ( $minutes_temp == '45' ) {
			$counter = 3;
		}
		$bookingTime = array();
		if ( $bookingduration <= $remainingtime ) {
			...
			for ( $i = 1; $i <= $totalDays; $i = $i + 1 ) {
				if ( $remainingduration > 0 ) {
					...
					// $actual_full_day;
					$arrBookingtimeNext = array(
						'booking_record_id' => $order_id,
						'booking_date'      => $nextDate,
						'booking_time_from' => '00:00',
						'booking_time_till' => '',
						'full_day'          => $actual_full_day,
						'created_date'      => $currentdate,
					);
	...
	function bkx_comments_clauses( $pieces, $wp_query ) {
		if ( is_admin() ) {
			...
			if ( isset( $current_screen->base ) && ( 'edit' === $current_screen->base || 'edit' === $current_screen->parent_base ) && 'bkx_booking' === $current_screen->post_type
				 || ( isset( $_POST['action'], $_POST['post_id'] ) && sanitize_text_field( wp_unslash( $_POST['action'] ) ) == 'bkx_action_view_summary' )
			) {
				global $wpdb;
	...
	public function get_step_2_booking_form_details( $args ) {
		...
		$base_name       = apply_filters( 'bkx_form_step_info_base', $bkx_base->get_title(), $bkx_base );
		$step_2_details .= "<dl>
	...
	public function display_availability_slots_html( $args ) {
			...
			if ( ! empty( $range ) ) {
				for ( $cell_start = $first; $cell_start < $last; $cell_start = $cell_start + $step ) {
					...
					if ( in_array( $counter, $range ) ) {
						...
						if ( isset( $is_time_zone_checking ) && $is_time_zone_checking == true
							 && strtotime( $slot_sys_time ) < strtotime( $current_time ) && date( 'Ymd' ) == date( 'Ymd', strtotime( $args['booking_date'] ) )
						) {
		...
		if ( isset( $base_time_option ) && $base_time_option == 'D' && isset( $base_day ) && $base_day > 0 ) {
			...
			echo $day_style_header . $results; // phpcs:disable WordPress.XSS.EscapeOutput.OutputNotEscaped
		}
	}
